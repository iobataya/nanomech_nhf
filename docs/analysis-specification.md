...
2026-09-14 モデル選択実装: `StaticConfig.model`（既定Hertz）と `--model` を追加。
Hertz、Sneddon、Pyramid、DMT_Sphere、DMT_Coneを選択可能。大文字小文字は区別しない。
円錐／四角錐は `cone_half_angle`（度、既定15）を使用。
全モデルはnm_modelsの基底クラス・解析ヤコビアン・スケーリングを使用する。
DMTのgammaは式の次元に基づき `adhesion_parameter_n_per_m` に保存する。
DMTの残差選択は旧ignore_baselineとは異なり全点を評価する。接触点推定によって
対象点を除外すると目的関数が不正に小さくなり得るため。接触初期値を複数試し、
収束かつフルランクの候補から最小残差を選ぶ。DMT_Sphereの接触境界は不連続であり、
ヤコビアンは境界以外の区分微分。旧式の物理的妥当性や旧CSVとの一致は別の検証事項。
...
2026-09-14 動的弾性率実装: Hertzの既記録式を用い、Piezo（流体抵抗補正のON/OFF）と
CleanDriveの複素弾性率を計算。pandasの `vea_results.csv` にstatic結果と併記する。
代表点0・500 HzはCleanDrive、貯蔵約4.151 MPa・損失約0.231 MPa。
旧CSVの約1.443 MPa・0.08061 MPaに対する1%比較は未合格であり、未解決差異として扱う。
既知複素弾性率の独立テストでは両方式で復元を確認。現実装はHertz・参照位相補正なし。

static初回実装の検証記録: Hertz球モデルをSI単位で実装し、Advance／Retractの
既知ヤング率2 MPaの合成曲線で相対誤差1e-5以内の復元を確認した。
代表sample点0は約2.313 MPaで、旧CSVの約0.100 MPaとの1%比較は未合格。
旧コードの `force*1e9` とSIモデルの混在を除き、残差とパラメータをスケーリングした。
...
統合前のstatic結果との相対差は1e-6以内。これは旧CSVとの1%比較の合格を意味しない。
