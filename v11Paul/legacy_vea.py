...
### VEA ENTRY POINT
# Legacy calculation functions and defaults above are intentionally unchanged.
if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Legacy VEA analysis with per-point CSV and GWY output")
    parser.add_argument("--calibration", "-c", type=pathlib.Path, required=True)
    parser.add_argument("--measurement", "-m", type=pathlib.Path, required=True)
    parser.add_argument("--output-dir", type=pathlib.Path,
                        help="Output directory (default: a unique run directory beside the sample)")
    parser.add_argument("--no-plots", action="store_true", help="Disable interactive plot windows")
    ...
    try:
        if not import_data(args.calibration, "Calibration"):
            raise RuntimeError("Failed to import calibration file")
        if not import_data(args.measurement, "Measurement"):
            raise RuntimeError("Failed to import measurement file")
        output_dir = args.output_dir
        if output_dir is None:
            run_stamp = datetime.datetime.now().strftime("%Y%m%d-%H%M%S-%f")
            output_dir = args.measurement.parent / (args.measurement.stem + "_legacy_vea_" + run_stamp)
        output_dir.mkdir(parents=True, exist_ok=False)
        # The original export code uses this variable for output paths and labels.
        path_of_the_directory = output_dir / args.measurement.name
        logging.info("Legacy VEA output directory: %s", output_dir.resolve())
