...
### EXCITATION-FIT ENTRY POINT
# Legacy calculation functions and defaults above are intentionally unchanged.
if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Legacy excitation coefficient calculation (point 0 only)")
    parser.add_argument("--calibration", "--input", "-c", type=pathlib.Path, required=True,
                        help="NHF file containing the excitation sweep")
    ...
    try:
        ...
        if not eval_coeffs_func():
            raise RuntimeError("Failed to evaluate excitation coefficients")
    except Exception:
        logging.exception("Legacy excitation-fit failed")
        sys.exit(1)
    print("AnalysisDone :)")
